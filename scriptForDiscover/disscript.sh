...
echo "Usage:"
echo "Stop bdna agent and remove install home:sh disscript.sh -k"
echo "Upload files to other servers: sh disscript.sh -f 'file_path'"

while getopts "kf:" opt; do
  case $opt in
    k)
      echo "this is -k: to kill all agents"
      ;;
    f)
  ...
  for (( i = 1 ; i < 6 ; i++ ))
    ...
    if [ $opt = "k" ];then
      ...
      echo "[`date`] Removing install_home on $target_host." | tee -a ./cacey.log
      ...
      cmd="scp -r $FILE  $BDNA_SERVER_OS_USERNAME@$target_host:/home/bdna/install773"
      echo "$cmd"
      `eval $cmd`
      if [ $? -gt 1 ]; then
